...
uint16_t slowDownDist =
    100; // the distance from which the image pixel should be tested and know whether it should slow down in advance
...
int obsta_overtake_status =
    0; // 0: not obstacle overtake 1: the previous obstacle is on the left 2: the previous obstacle is on the right
...
bool FindOneLeftEdge() {
  ...
  if (last.first - TuningVar::corner_range <= 0 || last.first + TuningVar::corner_range > WorldSize.w - 1
      || last.second - TuningVar::corner_range <= 0 || last.second + TuningVar::corner_range > WorldSize.h - 1)
    return true;
  if (left_edge.points.size() == 2) CornerCheck_left = findCorner_old(left_edge.points.back());
  else
    CornerCheck_left =
        findCorner_new(CornerCheck_left, left_edge.points[left_edge.points.size() - 2], left_edge.points.back());
  int total = pow(TuningVar::corner_range * 2 + 1, 2);
  //find corners
  if (left_edge.points.back().second <= WorldSize.h / TuningVar::corner_height_ratio && left_corners.size() < 40) {
    ...
    if (CornerCheck_left > total * TuningVar::corner_min / 100
      ...
      if (abs(last.first - left_corners.back().first) + abs(last.second - left_corners.back().second)
...
bool FindOneRightEdge() {
  ...
  if (right_edge.points.size() == 2) CornerCheck_right = findCorner_old(right_edge.points.back());
    ...
    CornerCheck_right =
        findCorner_new(CornerCheck_right, right_edge.points[right_edge.points.size() - 2], right_edge.points.back());
  int total = pow(TuningVar::corner_range * 2 + 1, 2);
  //find corners
  if (right_edge.points.back().second <= WorldSize.h / TuningVar::corner_height_ratio && right_corners.size() < 40) {

    //if in this threshold, consider as corner
    if (CornerCheck_right > total * TuningVar::corner_min / 100
        && CornerCheck_right < total * TuningVar::corner_max / 100) {
      if (abs(last.first - right_corners.back().first) + abs(last.second - right_corners.back().second)
          <= TuningVar::min_corners_dist) { //discard if too close
        return true;
      }
...
bool FindEdges() {
  ...
  while (left_edge.points.size() <= 50 && right_edge.points.size() <= 50 && (!flag_break_left || !flag_break_right)) {
    if (!flag_break_left)
    ...
    if (worldview::car1::transformMatrix[min(left_edge.points.back().first + 1, WorldSize.w - 1)][WorldSize.h
        - left_edge.points.back().second][0] == -1) {
      flag_break_left = true;
    }
    if (worldview::car1::transformMatrix[max(left_edge.points.back().first - 1, 1)][WorldSize.h
        - left_edge.points.back().second][0] == -1) {
      flag_break_left = true;
    }
    if (right_edge.points.back().second > TuningVar::edge_min_worldview_bound_check) {
      if (worldview::car1::transformMatrix[min(right_edge.points.back().first + 1, WorldSize.w - 1)][WorldSize.h
          - right_edge.points.back().second][0] == -1) {
        flag_break_right = true;
      }
      if (worldview::car1::transformMatrix[max(right_edge.points.back().first - 1, 1)][WorldSize.h
          - right_edge.points.back().second][0] == -1) {
        flag_break_right = true;
      }
    }
  ...
  if (obsta_status == ObstaclePos::kNull) {
    //case 1
    if (left_edge.size() >= 46 && getWorldBit(left_edge.points[45].first - 1, left_edge.points[45].second)
        == 1) { //only for left of left edge is black
      int cnt_black = 0;
    ...
    if (right_edge.size() >= 46 && getWorldBit(right_edge.points[45].first - 1, right_edge.points[45].second)
        == 1) { //only for right of right edge is black
      int cnt_black = 0;
      for (int i = 40; i < 45; i++)
        cnt_black += getWorldBit(right_edge.points[i].first - 7, right_edge.points[i].second);
      if (cnt_black == 5) {
    ...
    if (getWorldBit(left_edge.points.back().first - 1, left_edge.points.back().second) == 1 && !FindOneLeftEdge()) {
      int cnt_black = 0;
      for (int i = left_edge.size() - 5; i < left_edge.size(); i++)
        cnt_black += getWorldBit(left_edge.points[i].first + 7,
    ...
    if (getWorldBit(right_edge.points.back().first + 1, right_edge.points.back().second) == 1 && !FindOneRightEdge()) {
      int cnt_black = 0;
      for (int i = right_edge.size() - 5; i < right_edge.size(); i++)
        cnt_black += getWorldBit(right_edge.points[i].first - 7,
    ...
    if (left_corners.size() == 1 && right_corners.size() == 0
        && getWorldBit(left_edge.points.back().first - 1, left_edge.points.back().second)) {
      int cnt_black = 0;
      for (int i = left_edge.size() - 5; i < left_edge.size(); i++)
        cnt_black += getWorldBit(left_edge.points[i].first + 7,
                                 left_edge.points[i].second);
    ...
    if (right_corners.size() == 1 && left_corners.size() == 0
        && getWorldBit(right_edge.points.back().first - 1, right_edge.points.back().second)) {
      int cnt_black = 0;
      for (int i = right_edge.size() - 5; i < right_edge.size(); i++)
        cnt_black += getWorldBit(right_edge.points[i].first - 7,
                                 right_edge.points[i].second);
...
Feature featureIdent_Corner() {
  ...
  if (left_corners.size() > 0 && right_corners.size() > 0) {
    ...
    uint16_t cornerMid_x = (left_corners.front().first + right_corners.front().first) / 2; //corner midpoint x-cor
    uint16_t cornerMid_y = (left_corners.front().second + right_corners.front().second) / 2; //corner midpoint y-cor
    /*For identifying the feature*/
    uint16_t test_y = cornerMid_y + TuningVar::testDist;
    uint16_t test_x = (test_y - cornerMid_y) * (right_corners.front().second - left_corners.front().second)
...
//		if (is_round && roundaboutStatus == 0
    if (getWorldBit(test_x, test_y) && getWorldBit(test_x + 1, test_y) && getWorldBit(test_x, test_y + 1)
        && getWorldBit(test_x - 1, test_y) && roundaboutStatus == 0 && crossingStatus == 0/*Temporary close*/) {
      //All black
      if (abs(carMid.second - cornerMid_y) < TuningVar::action_distance) {
        if (is_front_car || pBT->getBufferFeature() == Feature::kRoundabout
            || TuningVar::single_car_testing) {// for single car running testing
          pBT->resetFeature();
          ...
          start_x = (start_y - (left_corners.front().second + right_corners.front().second) / 2)
              ...
              * (right_corners.front().second - left_corners.front().second)
              + (left_corners.front().first + right_corners.front().first) / 2;
    ...
    if (roundaboutStatus == 0 && crossingStatus
      ...
      if ((worldview::car1::transformMatrix[min(right_edge.points.back().first + 1, WorldSize.w - 1)][WorldSize.h
        ...
        if (abs(left_corners.front().second - carMid.second) <= TuningVar::min_dist_meet_crossing) {
          //push the midpoint of right edge into corner
          right_corners.push_back({(right_edge.points.front().first + right_edge.points.back().first) / 2,
                                   (right_edge.points.front().second + right_edge.points.back().second) / 2});
          crossing = true;
        }
      }
      //left edge touch left boundary)
      if ((worldview::car1::transformMatrix[max(left_edge.points.back().first - 1, 1)][WorldSize.h
          - left_edge.points.back().second][0] == -1) && right_corners.size() > 0) {
        //Only right corner
        if (abs(right_corners.front().second - carMid.second) <= TuningVar::min_dist_meet_crossing) {
          left_corners.push_back({(left_edge.points.front().first + left_edge.points.back().first) / 2,
                                  (left_edge.points.front().second + left_edge.points.back().second) / 2});
          crossing = true;
        }
      }
  ...
  if (temp_is_front ? !roundabout_shortest(TuningVar::roundabout_shortest_flag, roundabout_cnt - 1)
                    : roundabout_shortest(TuningVar::roundabout_shortest_flag, roundabout_cnt - 1)) {
  ...
  if (exit_round_ready && roundaboutStatus == 1 && abs(encoder_total_round)
    ...
    if (temp_is_front ? !roundabout_shortest(TuningVar::roundabout_shortest_flag, roundabout_cnt - 1)
      ...
      if (debug) {
        pLcd->SetRegion(Lcd::Rect(roundabout_nearest_corner_left.first,
                                  WorldSize.h - roundabout_nearest_corner_left.second - 1,
                                  4,
                                  4));
      ...
      if (debug) {
        pLcd->SetRegion(Lcd::Rect(roundabout_nearest_corner_right.first,
                                  WorldSize.h - roundabout_nearest_corner_right.second - 1,
                                  4,
                                  4));
        pLcd->FillColor(Lcd::kCyan);
      }
  ...
  if (is_front_car && sendFlag && obsta_status != ObstaclePos::kNull
      && abs(encoder_total_obstacle) >= TuningVar::obstacle_encoder_count / 2) {
...
void GenPath(Feature feature) {
  ...
  if (!finishObstacleBypass && !TuningVar::obstacle_mode
      && obstacle_hardcode_enc_curr < TuningVar::obstacle_hardcode_enc_total) {
    ...
    return;
  } else if (!finishObstacleBypass && !TuningVar::obstacle_mode
      && obstacle_hardcode_enc_curr >= TuningVar::obstacle_hardcode_enc_total) {
   ...
   * */
  if (crossingStatus == 1 && encoder_total_cross
  ...
  if (roundaboutStatus == 1 && abs(encoder_total_round)
      < TuningVar::round_encoder_count/*abs(System::Time() - feature_start_time) < TuningVar::feature_inside_time*/) {
    //		pEncoder0->Update();
    //		pEncoder1->Update();
    encoder_total_round +=
        curr_enc_val_left/*(pEncoder0->GetCount() + pEncoder1->GetCount()) / 2*/;//Because for exit/enter, the car will first left then right which destroy the encoder
  ...
  if (roundaboutExitStatus == 1 && abs(encoder_total_exit)
      < TuningVar::roundExit_encoder_count) {//TODO: Be care of back turning of motor when stop will affect encoder value
    //		pEncoder0->Update();
    //		pEncoder1->Update();
  ...
  if (TuningVar::obstacle_mode) {
    ...
    if (is_front_car && TuningVar::obsta_overtake_mode && obsta_overtake_status != 0) {
      ...
      if (obsta_overtake_status == 1) {
        ...
        } else if (stop_obsta_overtake && pBT->hasFinishedObstacleOvertake()
            && System::Time() - pBT->getObstacleOvertakeTime() >= TuningVar::overtake_interval_time) {
      ...
      if (left_size < right_size) {
        for (int i = 0; i < right_edge.size(); i++) {
          ...
          if (translate_flag == TranslateType::kLeftNull) {
            path.push(right_edge.points[i].first - shift_left_null, right_edge.points[i].second);
          } else if (translate_flag == TranslateType::kRightNull) {
            path.push(left_edge.points[i].first + shift_right_null, left_edge.points[i].second);
          } else {
            //if average
            ...
            if (stop_before_roundexit && overtake) {
              ...
              if (!pBT->hasFinishedOvertake() || (pBT->hasFinishedOvertake()
                ...
                pServo->SetDegree(util::clamp<uint16_t>(
                    servo_bounds.kCenter - (TuningVar::servo_roundabout_kp * curr_servo_error
                        + TuningVar::servo_normal_kd * (curr_servo_error - prev_servo_error)),
                    servo_bounds.kRightBound,
                    servo_bounds.kLeftBound));
              ...
              else if (pBT->hasFinishedOvertake()
                  && ((System::Time() - pBT->getOvertakeTime()) > TuningVar::overtake_interval_time)) {
...
//								System::DelayMs(TuningVar::overtake_interval_time);
              ...
              pServo->SetDegree(util::clamp<uint16_t>(
                  ...
                  servo_bounds.kRightBound,
                  servo_bounds.kLeftBound));
              pid_left.SetSetpoint(
                  TuningVar::targetSpeed_round * differential_left((pServo->GetDegree() - servo_bounds.kCenter) / 10));
              pid_right.SetSetpoint(
                  TuningVar::targetSpeed_round * differential_left((-pServo->GetDegree() + servo_bounds.kCenter) / 10));
            }
          }
...
//						}
              ...
              pServo->SetDegree(util::clamp<uint16_t>(
                  ...
                  servo_bounds.kLeftBound));
              pid_left.SetSetpoint(
                  TuningVar::targetSpeed_slow * differential_left((pServo->GetDegree() - servo_bounds.kCenter) / 10));
              pid_right.SetSetpoint(
                  TuningVar::targetSpeed_slow * differential_left((-pServo->GetDegree() + servo_bounds.kCenter) / 10));
            }
              ...
              pServo->SetDegree(util::clamp<uint16_t>(
                  ...
                  servo_bounds.kLeftBound));
              pid_left.SetSetpoint(
                  TuningVar::targetSpeed_normal * differential_left((pServo->GetDegree() - servo_bounds.kCenter) / 10));
              pid_right.SetSetpoint(TuningVar::targetSpeed_normal
                                        * differential_left((-pServo->GetDegree() + servo_bounds.kCenter) / 10));
            }
            ...
            pServo->SetDegree(util::clamp<uint16_t>(
                servo_bounds.kCenter - (TuningVar::servo_sharp_turn_kp * curr_servo_error
                    + TuningVar::servo_normal_kd * (curr_servo_error - prev_servo_error)),
                servo_bounds.kRightBound,
                servo_bounds.kLeftBound));
            pid_left.SetSetpoint(TuningVar::targetSpeed_sharp_turn
                                     * differential_left((pServo->GetDegree() - servo_bounds.kCenter) / 10));
            pid_right.SetSetpoint(TuningVar::targetSpeed_sharp_turn
                                      * differential_left((-pServo->GetDegree() + servo_bounds.kCenter) / 10));
          }
...
//						              pWriter->WriteString("Pstraight");
            ...
            if (left_edge.points.size() < 50 || right_edge.points.size() < 50) {
              ...
              pServo->SetDegree(util::clamp<uint16_t>(
                  servo_bounds.kCenter - (TuningVar::servo_straight_kp * curr_servo_error
                      + TuningVar::servo_normal_kd * (curr_servo_error - prev_servo_error)),
                  servo_bounds.kRightBound,
                  servo_bounds.kLeftBound));
              pid_left.SetSetpoint(
              ...
              for (int i = 34; i < 50; i++) {
                path.push((left_edge.points[i].first + right_edge.points[i].first) / 2,
                          (left_edge.points[i].second + right_edge.points[i].second) / 2);
              }
...
//							pWriter->WriteString(timestr);
              ...
              if (abs(further_servo_error) > 300 || need_slow_down) {
                ...
                pid_left.SetSetpoint(
                    TuningVar::targetSpeed_slow * differential_left((pServo->GetDegree() - servo_bounds.kCenter) / 10));
                pid_right.SetSetpoint(TuningVar::targetSpeed_slow
                                          * differential_left((-pServo->GetDegree() + servo_bounds.kCenter) / 10));
              }
...
//					              pWriter->WriteString("Straight");
            ...
            pServo->SetDegree(util::clamp<uint16_t>(
                servo_bounds.kCenter - (TuningVar::servo_normal_kp * curr_servo_error
                    + TuningVar::servo_normal_kd * (curr_servo_error - prev_servo_error)),
...
//					}
          if (met_stop_line || (stop_obsta_overtake && is_front_car && obsta_overtake_status != 0)) {
            ...
            pid_right.SetSetpoint(0);
//						char buf[10]={};
//						sprintf(buf," %d %d",met_stop_line, obsta_overtake_status);
//						pLcd->SetRegion(Lcd::Rect(1,1,120,30));
