...
void Moving::NormalMovingTestingVersion1(FutabaS3010& servo, St7735r& lcd) {
	...
	servo.SetDegree(ServoStraightDegree - (Average - 35) * ServoP);
}

void Moving::NormalMovingTestingVersion2(FutabaS3010& servo, St7735r& lcd) {
	// initialize LCD console
	LcdConsole::Config console_config;
	console_config.lcd = &lcd;
	LcdConsole console(console_config);
...
//	  LcdConsole console(console_config);
	...
	if (ext_camptr[Center[layer] - 1][layer] || ext_camptr[Center[layer]][layer]
		...
		//      lcd.FillColor(Lcd::kRed);
		if ((ServoStraightDegree - (Center[layer] - 37) * ServoP) > ServoLeftBoundary) {
			UpdatedDegree = ServoLeftBoundary;
		} else if ((ServoStraightDegree - (Center[layer] - 37) * ServoP) < ServoRightBoundary) {
			UpdatedDegree = ServoRightBoundary;
		} else {
			UpdatedDegree = ServoStraightDegree - (Center[layer] - 37) * ServoP;
		}
		servo.SetDegree(UpdatedDegree); //Action immediately for this special case
		return;
		//    }
...
//	  	string s = "Average: " + to_string(Average) + "\n";
//	  	console.WriteString(s.c_str());
	if ((ServoStraightDegree - (Average - 37) * ServoP) > ServoLeftBoundary) {
		UpdatedDegree = ServoLeftBoundary;
	} else if ((ServoStraightDegree - (Average - 37) * ServoP) < ServoRightBoundary) {
		UpdatedDegree = ServoRightBoundary;
	} else {
		UpdatedDegree = ServoStraightDegree - (Average - 37) * ServoP;
	}
	/*Servo Control*/
	servo.SetDegree(UpdatedDegree);
...
void Moving::NormalMovingTestingVersion6(FutabaS3010& servo, St7735r& lcd, AlternateMotor & motor_right, AlternateMotor& motor_left) {
//	 initialize LCD console
//	  LcdConsole::Config console_config;
//	  console_config.lcd = &lcd;
//	  LcdConsole console(console_config);
	...
	int Middle_xcor = 37;
	int ServoP;
	const int ServoP_Straight = 20;
	const int ServoP_NormalTurning = 40;
	const int ServoP_SharpTurning = 40;
	const int ServoP_Roundabout = 40;
...
/*---------------------------------------------Not Round Road : Straight + Normal Turning + Sharp Turning*/
	if(!RoundRoadNow){
		...
		if( (CurrentRoadWidth >= NormalRoadWidth_UpperBound) && (CurrentRoadWidth <= CrossingRoadWidth_LowerBound) ){
		...
		if(abs(Error) < 10){
			...
			int left_area_sum = 0;
			for(int y=30; y>20;y--){
				for(int x = Middle_xcor; x>Middle_xcor-15 ;x--){
...
//			        string s = "Left: " + to_string(left_area_sum) + "\nRight: " + to_string(right_area_sum);
//			        console.SetCursorRow(1);
//			        console.WriteString(s.c_str());
			/*Motor Control*/
			// Reduce speed in advance
			if(abs(left_area_sum - right_area_sum) > 50){
				motor_right.SetPower(100);
				motor_left.SetPower(100);
			}
			else{
		...
		/*Normal Turning*/
		else if((abs(Error) >= 10) && (abs(Error) <=20)){
			/*Motor Control*/
			motor_right.SetPower(200);
			motor_left.SetPower(200);
			/*Servo Control*/
...
//---------
/*Moving function*/
bool Moving::NormalMoving(FutabaS3010& servo) {
	...
	int Average = sum / 30;
	servo.SetDegree(ServoStraightDegree - (Average - 40) * ServoPara);
	return true;
}

bool Moving::RoundMoving(FutabaS3010& servo) {
	//TURN LEFT ANYWAY
	servo.SetDegree(ServoLeftBoundary);
	return true;
}

bool Moving::CrossingMoving(FutabaS3010& servo) {
	//GO STRAIGHT ANYWAY
	servo.SetDegree(ServoStraightDegree);
	return true;
}
