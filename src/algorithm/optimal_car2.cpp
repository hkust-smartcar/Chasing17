...
int16_t obstacle_hardcode_enc_total = 6300;
...
bool FindOneLeftEdge() {
  ...
  //find corners
  if (left_edge.points.back().second <= WorldSize.h / TuningVar::corner_height_ratio && left_edge.size() < 40) {
...
bool FindOneRightEdge() {
  ...
  int total = pow(TuningVar::corner_range * 2 + 1, 2);
  //find corners
  if (right_edge.points.back().second <= WorldSize.h / TuningVar::corner_height_ratio && right_edge.size() < 40) {
    //if in this threshold, consider as corner
...
bool FindEdges() {
  ...
  while (left_edge.points.size() <= 50 && right_edge.points.size() <= 50 && (!flag_break_left || !flag_break_right)) {
    ...
    if (worldview::car2::transformMatrix[min(left_edge.points.back().first + 1, WorldSize.w - 1)][WorldSize.h
        - left_edge.points.back().second][0] == -1) {
      flag_break_left = true;
    }
    if (worldview::car2::transformMatrix[max(left_edge.points.back().first - 1, 1)][WorldSize.h
        - left_edge.points.back().second][0] == -1) {
      flag_break_left = true;
    }
    if (right_edge.points.back().second > TuningVar::edge_min_worldview_bound_check) {
      if (worldview::car2::transformMatrix[min(right_edge.points.back().first + 1, WorldSize.w - 1)][WorldSize.h
          - right_edge.points.back().second][0] == -1) {
        flag_break_right = true;
      }
      if (worldview::car2::transformMatrix[max(right_edge.points.back().first - 1, 1)][WorldSize.h
          - right_edge.points.back().second][0] == -1) {
        flag_break_right = true;
      }
    }
...
//		} else if (is_cross && crossingStatus == 0
    ...
    if (roundaboutStatus == 0 && crossingStatus
      ...
      bool crossing = false;
      if ((worldview::car2::transformMatrix[min(right_edge.points.back().first + 1, WorldSize.w - 1)][WorldSize.h
          - right_edge.points.back().second][0] == -1) && left_corners.size() > 0) {
        //Only left corner
      ...
      if ((worldview::car2::transformMatrix[max(
          left_edge.points.back().first - 1, 1)][WorldSize.h - left_edge.points.back().second][0] == -1)
